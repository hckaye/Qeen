...
public class CertificateVerify : ITlsMessage
{
    public TlsMessageType Type => TlsMessageType.CertificateVerify;
    
    public SignatureScheme SignatureAlgorithm { get; set; }
    public byte[] Signature { get; set; } = Array.Empty<byte>();

    public void Encode(ref TlsWriter writer)
    {
        writer.WriteUInt16((ushort)SignatureAlgorithm);
    ...
    public static ITlsMessage Decode(ref TlsReader reader, TlsMessageType type)
    {
        ...
        {
            SignatureAlgorithm = (SignatureScheme)reader.ReadUInt16(),
            Signature = reader.ReadVector16().ToArray()
        };
    }
}
